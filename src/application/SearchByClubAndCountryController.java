...
public class SearchByClubAndCountryController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		FilteredList<Player> filterByClub = new FilteredList<>(players, p -> true);
		
		clubQuery.textProperty().addListener((observable, oldValue, newValue) -> {
			filterByClub.setPredicate(p -> {
				...
				if (p.getPlayerClub().toLowerCase().indexOf(lowerCaseFilter) != -1) {
					return true;
				}
				return false;
			});
		...
		FilteredList<Player> filterbyCountry = new FilteredList<>(filterByClub, p -> true);
		
		countryQuery.textProperty().addListener((observable, oldValue, newValue) -> {
			filterbyCountry.setPredicate(p -> {
				...
				if (p.getCountry().toLowerCase().indexOf(lowerCaseFilter) != -1) {
					return true;
				}
				return false;
			});
		...
		SortedList<Player> sortedList = new SortedList<>(filterbyCountry);
